...
// EVM guard — sembunyikan semua fitur Cosmos jika chain adalah EVM
const isEvm = computed(() =>
  blockchain.current?.networkType === 'evm-mainnet' ||
  blockchain.current?.networkType === 'evm-testnet'
);
...
<template>
  <div>
    <!-- ═══════════════════════════════════════════════
         EVM Dashboard — ditampilkan hanya untuk EVM chain
    ════════════════════════════════════════════════ -->
    <div v-if="isEvm" class="space-y-4">
      <!-- EVM Info Banner -->
      <div class="bg-base-100 rounded shadow p-6">
        <div class="flex items-center gap-4 mb-4">
          ...
          />
          <div>
            <h2 class="text-xl font-bold text-main">{{ blockchain.current?.prettyName }}</h2>
            <span class="text-xs px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300 font-medium">
              EVM Chain
            </span>
          </div>
        </div>
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Chain ini adalah EVM chain. Fitur Cosmos seperti staking, governance, dan IBC tidak tersedia.
          Gunakan menu <strong>Blocks</strong> dan <strong>Transactions</strong> untuk menjelajahi chain ini.
        </p>
      </div>

      <!-- Stats tetap ditampilkan jika ada -->
      <div
        v-if="store.stats && store.stats.length > 0"
      ...
      <!-- Coin info jika tersedia di coingecko -->
    ...
    <!-- ═══════════════════════════════════════════════
         COSMOS Dashboard — ditampilkan hanya untuk Cosmos chain
    ════════════════════════════════════════════════ -->
    <div v-else>
      <div v-if="coinInfo && coinInfo.name" class="bg-base-100 rounded shadow">
        <div class="grid grid-cols-2 md:grid-cols-3 p-4">
